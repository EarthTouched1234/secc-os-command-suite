...
// Open a clean, print-ready document for the message → browser "Save as PDF"
function exportMessagePdf(role: string, turn: number | undefined, text: string, iso: string) {
  const title = role === 'horhanis' ? `HORHANiS${turn ? ` #${turn}` : ''}` : role.toUpperCase()
  const when = new Date(iso).toLocaleString()
  const w = window.open('', '_blank')
  if (!w) return
  w.document.write(
    `<!doctype html><html><head><meta charset="utf-8"><title>${escapeHtml(title)} — ${escapeHtml(when)}</title>` +
    `<style>@page{margin:2cm}body{font-family:Georgia,'Times New Roman',serif;color:#111;line-height:1.6;` +
    `max-width:720px;margin:0 auto;padding:24px}header{border-bottom:2px solid #c79a1e;padding-bottom:10px;` +
    `margin-bottom:22px}h1{font-size:18px;margin:0;font-family:system-ui,sans-serif}.meta{color:#666;font-size:12px;` +
    `margin-top:4px;font-family:system-ui,sans-serif}.body{white-space:pre-wrap;word-break:break-word;font-size:14px}` +
    ...
    `<header><h1>${escapeHtml(title)}</h1><div class="meta">${escapeHtml(when)} · SECC-OS Command Center</div></header>` +
    `<div class="body">${escapeHtml(text)}</div>` +
    `<footer>Generated by SECC-OS Command Center · ${escapeHtml(when)}</footer></body></html>`
  )
  w.document.close()
  w.focus()
